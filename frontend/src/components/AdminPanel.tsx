...
const ReservationList: React.FC = () => {
    ...
    const fetchReservations = useCallback(async () => { setLoading(true); try { let url = '/reservations/'; const params = new URLSearchParams(); if (startDate) { params.append('start_date', format(startDate, 'yyyy-MM-dd')); } if (endDate) { params.append('end_date', format(endDate, 'yyyy-MM-dd')); } if (selectedStatuses.length > 0) { selectedStatuses.forEach(status => { params.append('status', status); }); } if (params.toString()) { url = `/reservations/?${params.toString()}`; } const response = await api.get<Reservation[]>(url); const sorted = response.data.sort((a, b) => new Date(b.start_time).getTime() - new Date(a.start_time).getTime()); setReservations(sorted); } catch (err) { setError('予約の読み込みに失敗しました。'); } finally { setLoading(false); } }, [startDate, endDate, selectedStatuses]);
    ...
    return (<div>
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6">
            ...
            <div className="flex items-center space-x-2">
                ...
                <DatePicker selected={startDate} onChange={(date: Date | null) => setStartDate(date)} dateFormat="yyyy/MM/dd" placeholderText="開始日を選択" className="p-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" isClearable locale="ja" selectsStart startDate={startDate} endDate={endDate} />
