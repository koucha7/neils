...
const MenuManagement: React.FC = () => {
    ...
    const ServiceForm = ({ service, onSave, onCancel }: { service: Partial<Service>, onSave: (s: Partial<Service>) => void, onCancel: () => void }) => {
        ...
        return (<form onSubmit={handleSubmit} className="bg-gray-50 p-4 my-4 rounded-lg shadow space-y-4">
            ...
            <div>
                ...
                <select name="duration_minutes" value={formData.duration_minutes || 30} onChange={handleChange} required className="w-full p-2 border rounded bg-white">{DURATION_CHOICES.map(d =>
                    <option key={d} value={d}>{d} 分</option>)}</select>
            </div>
    ...
    return (<div>
        <div className="flex justify-between items-center mb-6">
        ...
        </div> {editingService && <ServiceForm service={editingService} onSave={handleSave} onCancel={() => setEditingService(null)} />} <div className="bg-white p-4 rounded-lg shadow">
            <table className="w-full text-sm">
                ...
                <tbody> {services.map(service => (<tr key={service.id} className="border-b hover:bg-gray-50">
...
const AttendanceManagement: React.FC = () => {
    ...
    const handleDateClick = async (date: Date | null) => { if (!date) return; setSelectedDate(date); setIsSubmitting(true); try { const dateStr = format(date, 'yyyy-MM-dd'); const response = await api.get('admin-api/available-slots/', { params: { date: dateStr } }); setTimeSlots(response.data); setIsModalOpen(true); } catch (error) { alert('時間枠の読み込みに失敗しました。'); console.error(error); } finally { setIsSubmitting(false); } };
    ...
    return (<div>
        ...
        </div> {isModalOpen && selectedDate && (<div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
                <h4 className="text-lg font-bold mb-4">{format(selectedDate, 'yyyy年 M月 d日')} の受付時間設定</h4>
                <div className="max-h-96 overflow-y-auto grid grid-cols-3 sm:grid-cols-4 gap-2 border p-4 rounded-md"> {timeSlots.map(slot => (<label key={slot.time} className="flex items-center p-2 rounded-md hover:bg-gray-100 cursor-pointer">
...
const ReservationList: React.FC = () => {
    ...
    return (<div>
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6">
            ...
            </label>))} {selectedStatuses.length > 0 && (<button onClick={() => setSelectedStatuses([])} className="text-gray-500 hover:text-gray-700"> 全てクリア </button>)} </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
            <table className="min-w-full leading-normal hidden sm:table">
                <thead>
                ...
                </thead><tbody className="text-sm"> {reservations.map(r => (<tr key={r.id} className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => handleRowClick(r.reservation_number)}>
                    ...
                    <td className="p-2 space-x-2" onClick={(e) => e.stopPropagation()}> {r.status === 'pending' && (<button onClick={() => handleConfirm(r.reservation_number)} className="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600">確定</button>)} {r.status !== 'cancelled' && r.status !== 'completed' && (<button onClick={() => handleCancel(r.reservation_number)} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">取消</button>)} </td>
                </tr>))} </tbody>
            </table>
            <div className="sm:hidden space-y-4"> {reservations.map(r => (<div key={r.id} className="bg-white shadow rounded-md p-4 cursor-pointer" onClick={() => handleRowClick(r.reservation_number)}>
                ...
                <p className="text-sm font-semibold text-gray-600"> ステータス: <span className={`px-2 py-1 text-xs font-semibold rounded-full ${r.status === 'pending' ? 'bg-yellow-200 text-yellow-800' : r.status === 'confirmed' ? 'bg-green-200 text-green-800' : r.status === 'cancelled' ? 'bg-red-200 text-red-800' : 'bg-gray-200 text-gray-800'}`}>{r.status}</span>
                </p>
                <div className="mt-2 space-x-2" onClick={(e) => e.stopPropagation()}> {r.status === 'pending' && (<button onClick={() => handleConfirm(r.reservation_number)} className="bg-green-500 text-white px-2 py-1 text-xs rounded hover:bg-green-600">確定</button>)} {r.status !== 'cancelled' && r.status !== 'completed' && (<button onClick={() => handleCancel(r.reservation_number)} className="bg-red-500 text-white px-2 py-1 text-xs rounded hover:bg-red-600">取消</button>)} </div>
            </div>))} </div>
        </div>
    </div>);
};
    ...
    return (<div className="flex items-center justify-center min-h-screen bg-gray-100">
        <form onSubmit={handleSubmit} className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
            ...
            <div>
                ...
                <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required />
            </div>
            <div>
                <label className="block text-sm font-medium text-gray-700">パスワード</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required />
            </div> {error && <p className="text-red-500 text-sm text-center">{error}</p>} <button type="submit" className="w-full px-4 py-2 text-lg font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"> ログイン </button>
        </form>
    </div>);
};
